...
export class SucursalComponent implements OnInit {
  ...
  constructor(
    ...
    if (!this.params) {
      ...
      this.sucursalForm = this.formBuilder.group({
        ...
        cedula_trabajador_gerente: [
          this.params.cedula_trabajador_gerente,
          Validators.required,
        ],
                ...
                .subscribe({
                  next: (localidadesResponse: LocalidadesResponse) => {
                    ...
                    this.id_distrito = this.identificarDistrito(
                      this.params.distrito
                    );
                    this.sucursalForm.value.distrito = this.id_distrito;
                    this.sucursalForm.patchValue(this.sucursalForm.value, {
                      onlySelf: false,
                      emitEvent: true,
                    });
                  },
  ...
  protected minDateInicioGerencia() {
    if (!this.sucursalForm.value.fecha_apertura) {
      let inicioGerencia = new Date(
        this.maxDateApertura().year,
        this.maxDateApertura().month - 1,
        this.maxDateApertura().day
      );
      return {
        year: inicioGerencia.getFullYear(),
        month: inicioGerencia.getMonth() + 1,
        day: inicioGerencia.getDate(),
      };
    } else {
      let inicioGerencia = new Date(
        this.sucursalForm.value.fecha_apertura.year,
        this.sucursalForm.value.fecha_apertura.month - 1,
        this.sucursalForm.value.fecha_apertura.day
      );
      return {
        year: inicioGerencia.getFullYear(),
        month: inicioGerencia.getMonth() + 1,
  ...
  formatoSucursal(sucursalFormValues: any) {
    return this.formType === 'editar'
      ? ({
          ...
          cedula_trabajador_gerente:
            sucursalFormValues.cedula_trabajador_gerente,
          provincia: this.nombre_provincia,
          canton: this.nombre_canton,
          distrito: this.nombre_distrito,
          fecha_apertura: new Date(
            sucursalFormValues.fecha_apertura.year,
            sucursalFormValues.fecha_apertura.month - 1,
            sucursalFormValues.fecha_apertura.day
          ).toISOString(),
          fecha_inicio_gerencia: new Date(
            sucursalFormValues.fecha_inicio_gerencia.year,
            sucursalFormValues.fecha_inicio_gerencia.month - 1,
            sucursalFormValues.fecha_inicio_gerencia.day
          ).toISOString(),
        } as Sucursal)
      : ({
          nombre_sucursal: sucursalFormValues.nombre_sucursal,
